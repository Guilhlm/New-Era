...
type TrainingCopyDayDialogProps = {
  open: boolean;
...
export function TrainingCopyDayDialog({
  open,
  sourceWeekday,
  sourceWeekdayLabel,
  saving = false,
  onClose,
  onCopy,
}: TrainingCopyDayDialogProps) {
  const firstTarget = TRAINING_WEEKDAYS.find((day) => day.index !== sourceWeekday)?.index ?? 0;
  ...
  return (
    <NativeDialog open={open} onClose={onClose}>
      ...
      >
        <div>
          <p className={cn(typeClass.title, typeToneClass.default)}>Copy workout day</p>
          <p className={cn('mt-2', typeClass.body, typeToneClass.muted60)}>
            Copy every muscle group from {sourceWeekdayLabel} to another day or all days. Target
            days&apos; current workout will be replaced.
          </p>
        </div>
        ...
        <label className={walletDialogFieldClass}>
          ...
          >
            ...
            {TRAINING_WEEKDAYS.filter((day) => day.index !== sourceWeekday).map((day) => (
