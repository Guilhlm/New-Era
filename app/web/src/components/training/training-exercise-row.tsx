...
export function TrainingExerciseRow({ data, actions, ui }: TrainingExerciseRowProps) {
  return (
    <div className="flex items-center gap-3 rounded-xl bg-layer2-half px-4 py-3">
      ...
      <div className="hidden shrink-0 items-center gap-0 sm:flex">
        <span className="min-w-[4.5rem] px-3 text-center text-sm text-text/70">
          {formatWeightLabel(data.weightKg)}
        </span>
        <span className="h-6 w-px bg-layer2" aria-hidden />
        <span className="min-w-[5rem] px-3 text-center text-sm text-text/70">
          {formatSeriesLabel(data.series)}
        </span>
        <span className="h-6 w-px bg-layer2" aria-hidden />
        <span className="min-w-[5.5rem] px-3 text-center text-sm text-text/70">
          {formatRepsLabel(data.repsMin, data.repsMax)}
        </span>
      </div>
