...
function parseDayRange(dateRaw: string) {
...
export class WaterLogService {
  ...
  async getDayLog(userId: string, dateRaw: string) {
    const { start, end } = parseDayRange(dateRaw);
  ...
  async upsertDayLog(userId: string, body: UpsertWaterLogDto) {
    const { start, end, logDate } = parseDayRange(body.date);

    const existing = await this.prisma.waterLog.findFirst({
      where: {
        userId,
        logDate: { gte: start, lte: end },
      },
    ...
    const log = existing
      ? await this.prisma.waterLog.update({
